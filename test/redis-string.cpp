...
public:
  ...
  // 成员序列化函数
  json to_json() const { return json{{"id", id_}, {"price", price_}}; }

  // 成员反序列化函数
  static Product from_json(const json &j) {
    return Product(j.at("id").get<std::string>(), j.at("price").get<double>());
  }
};
...
int main() {
  ...
  reply = static_cast<redisReply *>(
      redisCommand(c, "MSET age 10 gender male work cooder"));
  std::cout << reply->str << std::endl;
  freeReplyObject(reply);
  ...
  // 设置一个可计算的变量
  reply = static_cast<redisReply *>(redisCommand(c, "SET counter 0"));
  std::cout << reply->str << std::endl;
  freeReplyObject(reply);
  ...
  std::thread t2([]() {
    ...
    redisReply *reply;
    for (int i = 0; i < 1000; ++i) {
      reply = static_cast<redisReply *>(redisCommand(c, "INCRBY counter 2"));
      std::cout << reply->integer << std::endl;
      freeReplyObject(reply);
    }
  });
  ...
  reply = static_cast<redisReply *>(redisCommand(c, "GET products:product_1"));
  if (reply != nullptr && reply->type == REDIS_REPLY_STRING)
    std::cout << reply->str << std::endl;
  else
    std::cout << "error" << std::endl;
